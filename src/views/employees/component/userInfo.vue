    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <upload-image ref="imagePic" :limit="1" />
        </el-form-item>
...
import { reqSaveUserDetailById, reqGetPersonalDetail, reqUpdatePersonal } from '@/api/employees'
export default {
  data() {
    return {
      EmployeeEnum, // 员工枚举数据
  ...
  methods: {
    ...
    async saveUser() {
      // 注意点:
    // 如果用户不传头像保存, fileList是一个空数组, fileList[0]取出来是undefined
    // undefiend.url会报错
    // 方式一:
      const fileList = this.$refs.staffPhoto.fileList[0]
      const staffPhoto = fileList && fileList.url
      // 方式二: 新玩法 友好点
      ...
      if (!isUploadAll) {
        this.$message.error('图片还在上传中, 请稍等~')
        return
      }
    ...
    async getPersonalDetail() {
      ...
      // 员工照片回显 data.staffPhoto
      this.$refs.imagePic.fileList = [
        { url: data.staffPhoto }
      ]
      this.formData = data
    },
    async savePersonal() {
      const imagePic = this.$refs.imagePic
      const fileList = imagePic.fileList
      const isUploadAll = imagePic.isUploadAll
      if (!isUploadAll) {
        this.$message.error('当前有文件正在上传中, 请稍后...')
        return
      }
